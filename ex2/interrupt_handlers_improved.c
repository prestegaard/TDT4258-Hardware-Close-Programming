#include <stdint.h>
#include <stdbool.h>
#include "efm32gg.h"

#include "gpio.h"
#include "timer.h"
#include "global_var.h"

const uint8_t sinewave[] =	/* 256 values */
{
	
};

volatile uint8_t toggle_flag = 0;
volatile uint16_t sample_number = 0;

void button_change()
{
	uint32_t buttons = button_status();
	leds_output((buttons << 8));

	if (buttons == button1) {
		mode = 1;
	} else if (buttons == button2) {
		mode = 2;
	} else if (buttons == button3) {
		mode = 3;
	} else if (buttons == button4) {
		mode = 4;
	} else if (buttons == button5) {
		mode = 5;
	} else if (buttons == button6) {
		mode = 6;
	} else if (buttons == button7) {
		mode = 7;
	} else if (buttons == button8) {
		mode = 8;
	} else {
		mode = 0;
	}
	if (mode != 0) {
		//Dissable energy mode when sound should be outputed
		*SCR &= ~(1 << 1);
		*SCR &= ~(1 << 2);
	}

}

/* TIMER1 interrupt handler */
void __attribute__ ((interrupt)) TIMER0_IRQHandler()
{
	//Iterate over sine wave table, output these values to DAC
	*DAC0_CH0DATA = (uint16_t) sinewave[sample_number] * volume;
	*DAC0_CH1DATA = (uint16_t) sinewave[sample_number] * volume;
	//Iterate by 4 to reduce time needed per sinewave. 
	sample_number += 4;
	if (sample_number > 256) {
		sample_number = 0;
	}

	*TIMER0_IFC = 1;	//clear interrupt

}


/* GPIO even pin interrupt handler */
void __attribute__ ((interrupt)) GPIO_EVEN_IRQHandler()
{
	*GPIO_IFC = *GPIO_IF;
	button_change();
}

/* GPIO odd pin interrupt handler */
void __attribute__ ((interrupt)) GPIO_ODD_IRQHandler()
{
	*GPIO_IFC = *GPIO_IF;
	button_change();
}
